...
try {
    ...
    foreach ($sig in $signatures) {
        [IO.File]::WriteAllText($sky, "prefix;$($sig.Needle);suffix", [Text.UTF8Encoding]::new($false))
        ...
        if (-not $patched.Contains('codex-cu-target-window-guard:v1')) { throw "Install ($($sig.Name)) did not add the guard marker." }
        if (-not $patched.Contains('method:"activate_window"')) { throw "Install ($($sig.Name)) did not add activation." }
        if (-not $patched.Contains('method:"get_window"')) { throw "Install ($($sig.Name)) did not add window rehydration." }

        & $installer -SkyPath $sky -Action Install | Out-Null
        & $installer -SkyPath $sky -Action Uninstall | Out-Null
        if ((Get-FileHash -LiteralPath $sky -Algorithm SHA256).Hash -ne $originalHash) { throw "Uninstall ($($sig.Name)) did not restore the original sky.js." }
        if ((Test-Path "$sky.codex-cu-target-window-guard.bak") -or (Test-Path "$sky.codex-cu-target-window-guard.install.json")) {
            throw "Uninstall ($($sig.Name)) left guard ownership files behind."
        }
        & $installer -SkyPath $sky -Action Uninstall | Out-Null
    }
